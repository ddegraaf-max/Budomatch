...
├─ .env.example
└─ public/
   ├─ index.html          # marketingsite (NL/PL, standaard Pools, foto's, chat, inlog/registratie)
   ├─ dashboard.html      # dashboard (klant/vakman) — volledig tweetalig NL/PL
   ├─ regulamin.html      # Regulamin (algemene voorwaarden, PL)
   ├─ prywatnosc.html     # Polityka prywatności (privacy/RODO, PL)
   └─ budomatch-logo.svg
```
...
2. Zet voor Polen de betaalmethoden **Card**, **Przelewy24 (p24)** en **BLIK** aan (Settings → Payment methods).
   PLN is de gebruikte valuta; het bedrag is 50,00 zł incl. 23% btw per lead.
...
- **Taal:** de site en het dashboard staan standaard op **Pools**; de taalkeuze (NL/PL) wordt onthouden via een cookie en is gedeeld tussen site en dashboard.
- **Beveiliging:** inlog-cookies krijgen automatisch de `Secure`-vlag achter HTTPS (Railway), zijn `HttpOnly` en `SameSite=Lax`.
- **Juridisch:** `regulamin.html` en `prywatnosc.html` bevatten je bedrijfsgegevens (NIP/REGON) en zijn opgesteld als degelijke basis — laat ze vóór livegang nog juridisch nakijken.
- **Facturen:** voor elke betaalde lead wordt automatisch een **Faktura VAT** aangemaakt met doorlopende nummering (FV/jaar/volgnummer), verkoper- (NIP/REGON) en kopergegevens (NIP/adres indien ingevuld) en de netto/btw/bruto-opbouw. Professionals downloaden de faktura (afdrukken of opslaan als PDF) vanuit hun facturatie-overzicht. De nummering reset per kalenderjaar.
