...
const LEAD_PRICE_VAT = +(LEAD_PRICE_GROSS - LEAD_PRICE_NET).toFixed(2);   // 9.35
const CURRENCY = 'pln';
...
const CATS_PL = "Dobudówka, Klimatyzacja, Architekt, Usuwanie azbestu, Łazienki, Brukarstwo, Piec/kocioł CO, Pokrycia dachowe, Lukarna, Okno dachowe, Elektryk, Bramy garażowe, Czyszczenie elewacji, Szklenie, Ogrodzenia, Zabezpieczenia antywłamaniowe, Izolacja, Kuchnie, Stolarka okienna, Stacje ładowania, Hydraulik, Dezynsekcja, Rolety, Malowanie, Drzwi przesuwne HS, Ogród zimowy, Tynkowanie, Układanie płytek, Magazyn energii, Schody, Zakładanie ogrodów, Pielęgnacja ogrodów, Wentylacja, Remont, Podłogi, Ogrzewanie podłogowe, Osuszanie/wilgoć, Pompa ciepła, Wellness/sauny, Fotowoltaika, Markizy/osłony";
...
app.post('/api/leads/:id/checkout', requireRole('pro'), async (req, res) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      mode: 'payment',
      payment_method_types: ['card', 'p24', 'blik'], // p24 = Przelewy24, blik populair in PL
      line_items: [{
        quantity: 1,
        price_data: {
          currency: CURRENCY,
          unit_amount: Math.round(LEAD_PRICE_GROSS * 100), // grosze, incl. 23% btw
          product_data: { name: `Budomatch lead: ${r.service}`, description: 'Ontgrendeling aanvraag (incl. 23% btw)' },
        },
      }],
...
function systemPrompt(lang, user, mode) {
  const cats = lang === 'pl' ? CATS_PL : CATS_NL;
  const who = user
    ? (lang === 'pl'
        ? `Rozmawiasz z zalogowanym ${user.role === 'pro' ? 'fachowcem' : 'klientem'}: ${user.name}.`
        : `Je praat met een ingelogde ${user.role === 'pro' ? 'vakman' : 'klant'}: ${user.name}.`)
    : '';
  if (lang === 'pl') {
    return `Jesteś asystentem AI platformy Budomatch — marketplace łączącego mieszkańców Polski ze sprawdzonymi, lokalnymi fachowcami budowlanymi. ${who}
Specjalizacje (41): ${cats}.
${mode === 'customer'
  ? 'Twoim celem jest pomóc klientowi sprawnie przejść proces: doprecyzować zlecenie (zakres, lokalizacja, termin, budżet orientacyjny), wybrać właściwą specjalizację i przygotować jasny opis zlecenia, który fachowcy zrozumieją. Gdy opis jest gotowy, podsumuj go zwięźle, aby klient mógł go wkleić do formularza.'
  : 'Pomagaj wybrać specjalizację, opisać zlecenie i wyjaśniaj jak działa Budomatch (klient publikuje zlecenie za darmo; fachowcy przysyłają wyceny).'}
Odpowiadaj po polsku, krótko, ciepło i konkretnie. Nie podawaj sztywnych cen — to fachowcy przysyłają indywidualne wyceny.`;
  }
...
app.post('/api/chat', async (req, res) => {
  try {
    const lang = req.body.lang === 'pl' ? 'pl' : 'nl';
